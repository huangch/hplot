"""Translate MCP-supplied dict args into an ``hplot`` CLI argv list.

The MCP tool input schema (see :mod:`hplot.mcp.schema`) uses argparse's
canonical parameter names (snake_case). The CLI itself accepts kebab-case
flags. This module looks up each parameter in the command table and emits
the correct ``--flag value`` (or bare ``--flag`` for boolean flags)
sequence, so a single generic adapter can drive every sub-command without
...
def _flag_for(param: dict[str, Any]) -> str:
    """Pick the long flag (``--foo-bar``) for a parameter."""
...
def args_to_argv(command: dict[str, Any], args: dict[str, Any]) -> list[str]:
    """Return ``[<subcommand>, --flag, value, ...]`` for ``hplot ...``.

    * Boolean flags are emitted as bare flags only when truthy.
    * Repeatable flags (``multiple=True``) are emitted once per value.
    * Fixed-``nargs`` flags (e.g. ``--groups LOW HIGH``) are emitted as a
      single flag followed by all values; a wrong value count raises.
    ...
    * Required parameters missing from ``args`` raise :class:`AdapterError`.
    ...
    for n, p in params.items():
        ...
        if p.get("multiple"):
            ...
            nargs = p.get("nargs")
            if nargs is not None and len(value) != nargs:
                raise AdapterError(
                    f"Parameter {n!r} of command {name!r} expects exactly "
                    f"{nargs} values; got {len(value)}: {list(value)}"
                )
            # The CLI uses ``nargs`` (not ``action="append"``), so the flag is
            # emitted ONCE followed by all its values: ``--targets a b``.

"""Background-job manager for long-running hplot subcommands.

Each job runs as a subprocess invocation of ``python -m hplot ...``,
which gives us clean cancellation via SIGINT (escalating to SIGTERM on a
second request) and no thread-safety concerns across runs.

hplot is pure CPU (no GPU/CUDA stack), so unlike the wsinsight sibling
there is no GPU pinning: the default pool size is 1 and can be raised
with ``hplot-mcp --max-concurrent N`` to run, e.g., several permutation
screens in parallel.
"""
...
class JobManager:
    """Fixed-size worker pool for ``hplot`` subprocess jobs (CPU only)."""

    def __init__(self, max_concurrent: Optional[int] = None) -> None:
        self.max_concurrent = max_concurrent if max_concurrent and max_concurrent > 0 else 1
        self._jobs: dict = {}
    ...
    def submit(self, command: str, argv_tail: list) -> str:
        """Schedule a new job. Returns the job_id immediately."""
        job_id = uuid.uuid4().hex[:12]
        argv = [sys.executable, "-m", "hplot"] + argv_tail
        state = JobState(id=job_id, command=command, argv=argv)
        with self._lock:
            self._jobs[job_id] = state
        threading.Thread(
            target=self._run_job, args=(state,), daemon=True, name=f"hplot-job-{job_id}"
        ).start()
        return job_id
    ...
    def logs(self, job_id: str, since_line: int = 0, max_lines: int = 500) -> Optional[dict]:

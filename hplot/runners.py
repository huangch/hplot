import os
from .core import HPlot

def run_hplot_batch(
    df,
    target_prop="target_prop",
    layer_col="layer",
    group_col=None,
    base_prop=None,
    distance_col=None,
    distance_unit=None,
    ci=0.95,
    output_dir="hplots",
    file_prefix="hplot",
    ci_show=True,
    file_format="svg",
    dpi=300,
):
    """
    根據群組自動產出多張 H-Plot 圖檔（每個 group 一張）
    """
    os.makedirs(output_dir, exist_ok=True)

    if group_col and group_col in df.columns:
        groups = df[group_col].unique()
    else:
        groups = ["all"]

    for group in groups:
        if group == "all":
            sub_df = df
        else:
            sub_df = df[df[group_col] == group]

        h = HPlot()
        h.fit(sub_df, target_prop=target_prop, layer_col=layer_col, group_col=group_col, base_prop=base_prop, distance_col=distance_col, distance_unit=distance_unit, ci=ci)
        h.plot(ci_show=ci_show)
        filename = os.path.join(output_dir, f"{file_prefix}_{group}.{file_format}")
        h.savefig(filename, dpi=dpi)